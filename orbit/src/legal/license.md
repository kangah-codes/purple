_Last updated: May 11, 2025_

Purple is a personal budgeting app developed and maintained by a solo developer. By using Purple, you agree to the following license terms:

## 1. Usage

You are free to:

- Use the app for personal or internal business purposes.
- Share the app with others.

You may not:

- Sell, rent, or sublicense the app.
- Reverse engineer, decompile, or modify the app without permission.
- Misrepresent the app as your own.

## 2. Modifications

The developer reserves the right to:

- Stop maintaining or developing Purple at any time.
- Refuse to fix bugs or add new features.
- Change these terms at any time with or without notice.

## 3. Disclaimer of Warranty

THE SOFTWARE IS PROVIDED “AS IS”, WITHOUT WARRANTY OF ANY KIND, EXPRESS OR IMPLIED.
USE IT AT YOUR OWN RISK.

## 4. Limitation of Liability

In no event shall the developer be liable for any damages arising from the use or inability to use the app, including but not limited to data loss, financial loss, nuclear war, or business interruption.

## 5. Licenses

This application was developed using [React Native](https://reactnative.dev/), an open-source framework maintained by Meta.

Purple also uses third-party assets and dependencies. All trademarks, copyrights, and licenses for these components are retained by their respective owners.

### Fonts
- [Satoshi Font](https://www.fontshare.com/fonts/satoshi) — © Indian Type Foundry. Licensed under the [Fontshare License](https://www.fontshare.com/licenses).

### Graphics
- Visual elements, illustrations, and icons used in the app may include assets sourced from:
  - [Untitled Icons](https://www.untitledui.com/download/untitled-ui) — © Untitled UI. Used under the Free License for commercial use. License details available on the [Untitled UI License Page](https://www.untitledui.com/license)
  - [Canva](https://canva.com) - Used under Canva’s [Content License Agreement](https://www.canva.com/policies/content-license-agreement/). Assets are incorporated into the app’s design and may not be extracted or reused outside the app.

### Dependencies
- Various open-source libraries provided by the JavaScript, TypeScript, and React Native communities under their respective open-source licenses (MIT, Apache, etc.).

Please refer to each package’s documentation or repository for full license details.

---
